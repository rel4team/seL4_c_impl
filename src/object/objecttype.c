...
// finaliseCap_ret_t finaliseCap(cap_t *_cap, bool_t final, bool_t exposed)
// {
//     cap_t cap = *_cap;
...
//     if (isArchCap(cap))
//     {
//         return Arch_finaliseCap((cap_t *)&cap, final);
//     }
...
//     case cap_thread_cap:
//     {
//         return process1(_cap, final, exposed);
//         if (final)
//         {
//             tcb_t *tcb;
//             cte_t *cte_ptr;
...
cap_t CONST updateCapData(bool_t preserve, word_t newData, cap_t *_cap)
{
    cap_t cap = *_cap;
    if (isArchCap(cap))
    {
    ...
    case cap_endpoint_cap:
        if (!preserve && cap_endpoint_cap_get_capEPBadge(cap) == 0)
        {
            ...
            cap_t cap1 = cap_endpoint_cap_set_capEPBadge(cap, newData);
            printf(" endpoint new %llx %llx \n", (unsigned long long)cap.words[0], (unsigned long long)cap.words[1]);
            printf(" endpoint %llx %llx \n", (unsigned long long)cap1.words[0], (unsigned long long)cap1.words[1]);
    ...
    case cap_notification_cap:
        // printf("prepare in here\n");
        // return process1(preserve, newData, _cap);
        if (!preserve && cap_notification_cap_get_capNtfnBadge(cap) == 0)
        {
            // return cap_notification_cap_set_capNtfnBadge(cap, newData);
            cap_t cap1 = cap_notification_cap_set_capNtfnBadge(cap, newData);
            printf(" notification new %llx %llx \n", (unsigned long long)cap.words[0], (unsigned long long)cap.words[1]);
            printf(" notification %llx %llx \n", (unsigned long long)cap1.words[0], (unsigned long long)cap1.words[1]);
    ...
    case cap_cnode_cap:
    {
        ...
        else
        {
            ...
            new_cap = cap_cnode_cap_set_capCNodeGuardSize(new_cap,
                                                          guardSize);
            printf("here guard %llx guardSize %llx\n", (unsigned long long)guard, (unsigned long long)guardSize);
            printf(" cnode new %llx %llx \n", (unsigned long long)cap.words[0], (unsigned long long)cap.words[1]);
            printf(" cnode %llx %llx \n", (unsigned long long)new_cap.words[0], (unsigned long long)new_cap.words[1]);
...
cap_t CONST maskCapRights(seL4_CapRights_t cap_rights, cap_t *_cap)
{
    cap_t cap = *_cap;
    if (isArchCap(cap))
    {
    ...
    case cap_endpoint_cap:
    {
        ...
        // unsigned long long addr =(unsigned long long)&new_cap;
        // return process1( (cap_t *)addr);
        return new_cap;
    }
...
// {
//     // printf("%llx %llx %llx %llx %llx %llx %llx %llx\n",(unsigned long long)t,(unsigned long long)parent,(unsigned long long)destCNode,(unsigned long long)destOffset,(unsigned long long)destLength,(unsigned long long)regionBase,(unsigned long long)userSize,(unsigned long long)deviceMemory);
//     word_t objectSize;
//     void *nextFreeArea;
//     word_t i;
...
//         // printf("val :%llx %llx %llx %llx\n", (unsigned long long)((word_t)nextFreeArea + (i << objectSize)), (unsigned long long)&destCNode[destOffset + i], (unsigned long long)cap.words[0], (unsigned long long)cap.words[1]);
//         /* Insert the cap into the user's cspace. */
//         insertNewCap(parent, &destCNode[destOffset + i], (cap_t *)&cap);
...
//         return EXCEPTION_SYSCALL_ERRcap
...
//         }
// #endif
//         return decodeTCBInvocation(invLabel, length, (cap_t *)&cap, slot, call, buffer);
...
//         }
// #endif
//         return decodeCNodeInvocation(invLabel, length, (cap_t *)&cap, buffer);

//     case cap_untyped_cap:
//         return decodeUntypedInvocation(invLabel, length, slot,(cap_t *)& cap, call, buffer);
