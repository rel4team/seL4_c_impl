...
//                 {
//                     bool_t canDonate = sender->tcbSchedContext != NULL && seL4_Fault_get_seL4_FaultType(sender->tcbFault) != seL4_Fault_Timeout;
...
//     setRegister(thread, msgInfoRegister, wordFromMessageInfo(seL4_MessageInfo_new(current_syscall_error.type, 0, 0, len)));
// }
...
//         callerCap =process1(tptr);
//         printf("before :%llx\n",(unsigned long long) callerCap);
...
//     case EPState_Send:
//     { 
//         tcb_queue_t queue = process2(epptr,badge);
// //         tcb_t *thread, *next; 
