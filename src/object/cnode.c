...
finaliseSlot_ret_t finaliseSlot(cte_t *slot, bool_t exposed);
...
// exception_t process2(word_t *buffer, cte_t *srcSlot, cte_t *desSlot, cap_t *c1, cap_t *c2);

// exception_t decodeCNodeInvocation(word_t invLabel, word_t length, cap_t *_cap,
//                                    word_t *buffer)
// {
//     cap_t cap = *_cap;
...
//             srcCap = maskCapRights(cap_rights, (cap_t *)&srcSlot->cap);
//             dc_ret = deriveCap(srcSlot, (cap_t *)&srcCap);
...
//             srcCap = maskCapRights(cap_rights, (cap_t *)&srcSlot->cap);
//             cap_t cap1 = updateCapData(false, capData, (cap_t *)&srcCap);
...
//             newCap = dc_ret.cap;
//             isMove = false;
//             // return process2(buffer, srcSlot, destSlot, (cap_t *)&srcCap, (cap_t *)0);
//             break;
...
//             newCap = updateCapData(true, capData, (cap_t *)&srcSlot->cap);
//             printf("newCap :%llx %llx\n", (unsigned long long)newCap.words[0], (unsigned long long)newCap.words[1]);
//             isMove = true;
...
//         if (isMove)
//         {
//             return invokeCNodeMove((cap_t *)&newCap, srcSlot, destSlot);
//         }
//         else
//         {
//             return invokeCNodeInsert((cap_t *)&newCap, srcSlot, destSlot);
//         }
//     }
...
//         }
//         setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
//         return invokeCNodeCancelBadgedSends((cap_t *)&destCap);
//     }
...
//         if (length < 8 || current_extra_caps.excaprefs[0] == NULL || current_extra_caps.excaprefs[1] == NULL)
//         {
//             current_syscall_error.type = seL4_TruncatedMessage;
//             return EXCEPTION_SYSCALL_ERROR;
//         }
...
//         newSrcCap = updateCapData(true, srcNewData, (cap_t *)&srcSlot->cap);

//         printf("newSrcCap :%llx %llx\n", (unsigned long long)newSrcCap.words[0], (unsigned long long)newSrcCap.words[1]);
//         newPivotCap = updateCapData(true, pivotNewData, (cap_t *)&pivotSlot->cap);

//         printf("newPivotCap :%llx %llx\n", (unsigned long long)newPivotCap.words[0], (unsigned long long)newPivotCap.words[1]);
...
//         setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
//         return invokeCNodeRotate((cap_t *)&newSrcCap, (cap_t *)&newPivotCap,
...
// {
//     cteInsert((cap_t *)&cap, srcSlot, destSlot);
...
// {
//     cteMove((cap_t *)&cap, srcSlot, destSlot);
...
//     if (slot1 == slot3)
//     {
//         cteSwap((cap_t *)&cap1, slot1, (cap_t *)&cap2, slot2);
//     }
//     else
//     {
//         cteMove((cap_t *)&cap2, slot2, slot3);
//         cteMove((cap_t *)&cap1, slot1, slot2);
//     }
 ...
 * parent.
 */
__attribute__((unused)) static void setUntypedCapAsFull(cap_t srcCap, cap_t newCap, cte_t *srcSlot)
{
    if ((cap_get_capType(srcCap) == cap_untyped_cap) && (cap_get_capType(newCap) == cap_untyped_cap))
    {
        if ((cap_untyped_cap_get_capPtr(srcCap) == cap_untyped_cap_get_capPtr(newCap)) && (cap_untyped_cap_get_capBlockSize(newCap) == cap_untyped_cap_get_capBlockSize(srcCap)))
        {
...
// void process1(cap_t *_newCap, cte_t *srcSlot, cte_t *destSlot, mdb_node_t *d, cap_t *c);
// void cteInsert(cap_t *_newCap, cte_t *srcSlot, cte_t *destSlot)
// {

//     cap_t newCap = *_newCap;
...
//     process1(_newCap, srcSlot, destSlot, (mdb_node_t *)&newMDB,(cap_t*)& srcCap);
...
// }
// void process1(cap_t *_cap1, cte_t *slot1, cap_t *_cap2, cte_t *slot2, word_t p1, word_t p2, mdb_node_t *m, mdb_node_t *m2);
// void cteSwap(cap_t *_cap1, cte_t *slot1, cap_t *_cap2, cte_t *slot2)
// {
//     // cap_t cap1 = *_cap1;
//     // cap_t cap2 = *_cap2;
//     // printf(" ccteSwap: %llx %llx %llx %llx\n", (unsigned long long)cap1.words[0], (unsigned long long)cap1.words[1], (unsigned long long)cap2.words[0], (unsigned long long)cap2.words[1]);
...
//     process1(_cap1, slot1, _cap2, slot2, prev_ptr, next_ptr, (mdb_node_t *)&mdb1, (mdb_node_t *)&mdb2);
...
//     {
//         emptySlot(slot, (cap_t*)&fs_ret.cleanupInfo);
//     }
//     return EXCEPTION_NONE;
// }
// void emptySlot(cte_t *slot, cap_t cleanupInfo)
...
// }
//  finaliseSlot_ret_t finaliseSlot(cte_t *slot, bool_t immediate);
// finaliseSlot_ret_t finaliseSlot(cte_t *slot, bool_t immediate)
// {
//     bool_t final;
...
//         fc_ret = finaliseCap((cap_t*)&slot->cap, final, false);

//         if (capRemovable((cap_t *)&fc_ret.remainder, slot))
//         {
...
//         if (!immediate && capCyclicZombie((cap_t *)&fc_ret.remainder, slot))
//         {
//             ret.status = EXCEPTION_NONE;
//             ret.success = false;
//             ret.cleanupInfo = fc_ret.cleanupInfo;
...
//         final = isFinalCapability(slot);
//         fc_ret = finaliseCap((cap_t *)&slot->cap, final, true);
//         /* Haskell error: "cteDeleteOne: cap should be removable" */
//         assert(capRemovable((cap_t *)&fc_ret.remainder, slot) &&
//                cap_get_capType(fc_ret.cleanupInfo) == cap_null_cap);
//                cap_t cap1=cap_null_cap_new();
//         emptySlot(slot, (cap_t *)&cap1);
//     }
// }
