...
//     status = lookupExtraCaps(thread, buffer, (seL4_MessageInfo_t *)&info);
...
// #else
//     status = decodeInvocation(seL4_MessageInfo_get_label(info), length,
//                               cptr, lu_ret.slot, (cap_t*)& lu_ret.cap,
...
//         receiveIPC(NODE_STATE(ksCurThread), (cap_t *)&lu_ret.cap, isBlocking);
// #endif
//         break;

//     case cap_notification_cap:
//     {
//         handleRecv1(isBlocking);
//         // notification_t *ntfnPtr;
//         // tcb_t *boundTCB;
//         // ntfnPtr = NTFN_PTR(cap_notification_cap_get_capNtfnPtr(lu_ret.cap));
//         // boundTCB = (tcb_t *)notification_ptr_get_ntfnBoundTCB(ntfnPtr);
//         // if (unlikely(!cap_notification_cap_get_capNtfnCanReceive(lu_ret.cap) || (boundTCB && boundTCB != NODE_STATE(ksCurThread))))
//         // {
//         //     current_lookup_fault = lookup_fault_missing_capability_new(0);
//         //     current_fault = seL4_Fault_CapFault_new(epCPtr, true);
//         //     handleFault(NODE_STATE(ksCurThread));
...
//         // receiveSignal(NODE_STATE(ksCurThread), (cap_t *)&lu_ret.cap, isBlocking);
//         break;
//     }
//     default:
//         current_lookup_fault = lookup_fault_missing_capability_new(0);
