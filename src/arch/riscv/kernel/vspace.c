...
//     printf("vspace root %llx\n",(unsigned long long ) vspace_root);
//     return ret;
// }
...
exception_t decodeRISCVPageTableInvocation(word_t label, word_t length,
...
//         return performPageTableInvocationUnmap((cap_t*)&cap, cte);
//     }
...
//     setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
//     return performPageTableInvocationMap((cap_t*)&cap, cte, pte, ptSlot);
// }
 

 exception_t decodeRISCVFrameInvocation(word_t label, word_t length,
...
//             printf("%llx %llx\n",(unsigned long long) mapped_vaddr , (unsigned long long) vaddr);
...
//         // printf("vcaddr %llx %llx %llx %llx %llx\n",(unsigned long long)vmRights,(unsigned long long)frame_paddr,(unsigned long long)cap.words[0],(unsigned long long)cap.words[1],(unsigned long long)executable);
//         pte_t pte = makeUserPTE(frame_paddr, executable, vmRights);
//         setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
//         return performPageInvocationMapPTE((cap_t *)&cap, cte, pte, lu_ret.ptSlot);
//     }
    
//     // case RISCVPageUnmap:
//     // {
//     //     setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
//     //     return performPageInvocationUnmap((cap_t*)&cap, cte);
//     // }
...
//     default:{
//         exception_t res= process(label,(cap_t*)& cap,cte,call);
//         return res;
//     }
...
//         return decodeRISCVPageTableInvocation(label, length, cte, (cap_t*)&cap, buffer);

//     case cap_frame_cap:
//         return decodeRISCVFrameInvocation(label, length, cte, (cap_t*)&cap, call, buffer);
...
//         if (length < 2 || current_extra_caps.excaprefs[0] == NULL || current_extra_caps.excaprefs[1] == NULL)
//         {
//             current_syscall_error.type = seL4_TruncatedMessage;
//             return EXCEPTION_SYSCALL_ERROR;
//         }
