...
__attribute__((unused)) BOOT_CODE static void init_irqs(cap_t root_cnode_cap)
{
    irq_t i;
...
__attribute__((unused)) BOOT_CODE static void init_cpu(void)
{
...
)
{
    ...
    result = rust_try_init_kernel(ui_p_reg_start,
                                  ...
                                  dtb_size);
    // printf("rootserver:\n");
    // printf("%llx\n", (unsigned long long)rootserver.cnode);
    // printf("%llx\n", (unsigned long long)rootserver.vspace);
    // printf("%llx\n", (unsigned long long)rootserver.asid_pool);
    // printf("%llx\n", (unsigned long long)rootserver.boot_info);
    // printf("%llx\n", (unsigned long long)rootserver.extra_bi);
    // // printf("%llx\n", (unsigned long long)rootserver.paging);
    // printf("%llx\n", (unsigned long long)rootserver.tcb);

    // printf("ndks_boot_t\n");
    // for (int i = 0; i < ndks_boot.resv_count; i++)
    // {
    //     printf("%llx %llx\n", (unsigned long long)ndks_boot.reserved[i].start,(unsigned long long) ndks_boot.reserved[i].end);
    // }
    // printf("----------------------------\n");
    // for (int i = 0; i < 16; i++)
    // {
    //     printf("%llx %llx\n", (unsigned long long)ndks_boot.freemem[i].start,(unsigned long long) ndks_boot.freemem[i].end);
    // }
    // printf("%llx\n", (unsigned long long)ndks_boot.bi_frame);
    // printf("%llx\n", (unsigned long long)ndks_boot.slot_pos_cur);
    // cap_t cptr = TCB_PTR_CTE_PTR(rootserver.tcb, tcbCTable)->cap;
    // cap_t bptr =TCB_PTR_CTE_PTR(rootserver.tcb, tcbBuffer)->cap;
    // printf("%llx %llx\n", (unsigned long long)cptr.words[0],(unsigned long long) cptr.words[1]);
    // printf("%llx %llx\n", (unsigned long long)bptr.words[0],(unsigned long long) bptr.words[1]);

    // seL4_BootInfo *bi = BI_PTR(rootserver.boot_info);
    // printf("%llx\n",(unsigned long long)bi->initThreadCNodeSizeBits);
    // printf("%llx\n",(unsigned long long)bi->initThreadDomain);
