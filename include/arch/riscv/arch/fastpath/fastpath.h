...
static inline
    ...
    void
    fastpath_reply_recv(word_t cptr, word_t r_msgInfo, word_t reply)
#else
    void
    fastpath_reply_recv(word_t cptr, word_t r_msgInfo)
#endif
        NORETURN;
...
void switchToThread_fp(tcb_t *thread, pte_t *vroot, pte_t stored_hw_asid);
...
void mdb_node_ptr_mset_mdbNext_mdbRevocable_mdbFirstBadged(
...
// }
void mdb_node_ptr_set_mdbPrev_np(mdb_node_t *node_ptr, word_t mdbPrev);
// static inline void mdb_node_ptr_set_mdbPrev_np(mdb_node_t *node_ptr, word_t mdbPrev)
// {
//     node_ptr->words[0] = mdbPrev;
// }
 bool_t isValidVTableRoot_fp(cap_t* vspace_root_cap);
...
/* This is an accelerated check that msgLength, which appears
   ...
   int fastpath_mi_check(word_t msgInfo);
// compile_assert(n_msgRegisters_eq_4, n_msgRegisters == 4) static inline int fastpath_mi_check(word_t msgInfo)
// {
//     return (msgInfo & MASK(seL4_MsgLengthBits + seL4_MsgExtraCapBits)) > 4;
// }
void fastpath_copy_mrs(word_t length, tcb_t *src, tcb_t *dest);
...
//     }
// }
int fastpath_reply_cap_check(cap_t *cap);
...
// {
//     NODE_UNLOCK_IF_HELD;
// 	printf("go into fastpath_restore %d %d\n",badge, msgInfo);
